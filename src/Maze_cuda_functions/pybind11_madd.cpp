...
extern void copyCuda(int* maze_array, int width, int height, int start_x, int start_y, int end_x, int end_y, unsigned long long seed);
...
extern void randomizeZerosCuda_ctrl(int* A, int X, int Y, float percentage, unsigned long long seed);
...
py::array_t<int> randomizeZeroswrapper_ctrl(py::array_t<int> result_array, int x, int y, float percentage, unsigned long long seed) {
    auto buf1 = result_array.request();
...
py::array_t<int> copyWrapper(py::array_t<int> result_array, int x, int y, py::tuple start_coord, py::tuple end_coord, unsigned long long seed) {
    auto buf1 = result_array.request();
    ...
    copyCuda(A, x, y, start_x, start_y, end_x, end_y, seed);
...
PYBIND11_MODULE(cu_matrix_add, m) {
    ...
    m.def("randomizeZerosCuda_ctrl", &randomizeZeroswrapper_ctrl, "Randomly turn a percentage of 0's to 2's with CUDA (ctrl)");
